    ...
    <div class="d-flex justify-content-between align-items-end">
        ...
        @if ($nombre == 'varios')
            @if($titulo == "Gastos de " . $nombre . " del día")
                <h1 class="mt-2 mb-3">{{ "Gastos varios del día" }}</h1>
            @else
            ...
            <form class="form-inline" method="POST" action="{{ url('admin/control/') }}">
                ...
                @if($titulo == "Gastos de " . $nombre . " del día")
                    <div class="form-group mx-sm-3 mb-2">
                        ...
                        <input type="number" min="0" required class="form-control" name="monto" placeholder="Cargar gasto">
                        <input type="hidden" class="form-control" name="id_desc" value="{{ $id_desc }}">
                        @if ($nombre == 'varios')
                        <input type="text" class="form-control" name="detalle" placeholder="Cargar detalle del gasto" value="Gastos {{$nombre}}">
                        @else
